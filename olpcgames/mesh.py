...
the dbus_get_object( handle, path ) function, which looks up an object 
(by DBUS path) shared by the user "handle" and returns a 
DBUS/Telepathy proxy for that object.  Note that this object *must*
work inside the GTK mainloop.  You *cannot* make synchronous 
calls on these objects!  You must use the named arguments:

    reply_handler, error_handler

for every call which you perform on a shared object (normally 
these are ExportedGObject instances).  You can use the 

    dbus_get_object_proxy( handle, path )
    
to retrieve a wrapped version of the proxy which allows your 
callbacks to run in the Pygame event loop, instead of the GTK 
mainloop.
...
Note: 
    ...
    The intention is that mesh will be heavily refactored, possibly as a 
...
def dbus_get_object(handle, path):
    ...
    it from Pygame will cause crashes!
    '''
...
def dbus_get_object_proxy(handle, path):
    '''Get a D-bus object from another participant for use in Pygame
    ...
    object on a participant, call this method.
    '''
    log.debug( 'dbus_get_object_proxy: %s %s', handle, path )
